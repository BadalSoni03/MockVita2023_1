...
inline void getMeTheSolution(ll testCase) {
	...
	stringstream ss(str);
	str.clear();
	while (getline(ss , str , ' ')) coords.pb(stoLL(str));

	ll start = 0 , end = 9;
	while (start <= end) {
		ll sum = start + end;
		if (sum & 1) swap(grid[start] , grid[end]);
		start++ , end--;
	}
	for (ll i = 0 ; i < 10 ; i++) {
		if (i & 1) reverse(all(grid[i]));
	}
	HashMap<ll , ll , custom_hash> Ladders , Snakes;
	ll cnt = 0;
	for (ll i = 0 ; i < 10 ; i++) {
		for (ll j = 0 ; j < 10 ; j++) {
			auto cell = grid[i][j];
			if (cell == "S(Start)") {}
				...
				string tmp = "";
				for (ll i = 2 ; i < cell.length() - 1 ; i++) tmp += cell[i];
				Ladders[cnt] = stoLL(tmp);
			}
			else if (cell[0] == 'S' and cell != "Start") {
				string tmp = "";
				for (ll i = 2 ; i < cell.length() - 1 ; i++) tmp += cell[i];
				Snakes[cnt] = stoLL(tmp);
			}
			cnt++;
		}
	}
	start = -1;
	ll snakes = 0 , ladders = 0;
	bool reached = false;

	auto fetch = [&](string & str) {
		string tmp = "";
		for (ll i = 2 ; i < str.length() - 1 ; i++) tmp += str[i];
		return stoLL(tmp) - 1;
	};

	for (auto & it : coords) --it; 
	for (ll i = 0 ; i < coords.size() ; i++) {
		...
		cerr << "Start : " << start sp << "Cell : " << cell nl;
		if (Snakes.count(start)) snakes++ , start = fetch(cell);
		else if (Ladders.count(start)) ladders++ , start = fetch(cell);
		if (start == 99) {
			reached = true;
			break;
		}
	...
	ans.pb(snakes);
	ans.pb(ladders);
	if (reached) cout << "Possible" sp << ans nl;
	else cout << "Not possible" sp << ans << start + 1 nl;
}
