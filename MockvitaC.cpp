...
inline void getMeTheSolution(ll testCase) {
	...
	cin >> num >> oprs;
	ll itr = 0;
	for (ll i = 0 ; i < oprs.length() ; i++) {
		char ch = oprs[i];
		if (ch == 'R') itr++;
		else if (ch == 'L') itr--;
		else if (ch == 'T' and itr >= 0 and itr < num.length()) (int)num[itr]++;
		else if (ch == 'D' and itr >= 0 and itr < num.length()) (int)num[itr]--;
		else if (ch == 'S') {
			ll idx = oprs[i + 1] - '0';
			idx-- , i++;
			swap(num[itr] , num[idx]);
		}
		cerr << num nl;
	}
	cout << num nl;
